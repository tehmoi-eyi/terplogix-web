  ...
  return (
    ...
    >
      ...
      <HeroPanel className="left-0 top-10 hidden w-56 sm:block" delay={0.2}>
        <MiniStatus title="COA Review" value="High Confidence" icon={FileText} />
        <div className="mt-4 space-y-2">
          <Meter label="Cannabinoids" value="92%" />
          <Meter label="Terpenes" value="84%" />
          <Meter label="Retail fit" value="78%" />
        </div>
      </HeroPanel>

      <HeroPanel className="right-0 top-20 hidden w-60 sm:block" delay={0.34}>
        <MiniStatus title="Serve Queue" value="Handoff Sent" icon={Send} />
        <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-muted">
          Goal: relaxation<br />
          Shortlist: 3 products<br />
          Staff context ready
        </div>
      </HeroPanel>

      <HeroPanel className="bottom-20 left-2 hidden w-64 md:block" delay={0.46}>
        <MiniStatus title="Live Menu" value="Synced" icon={MonitorUp} />
        <div className="mt-4 grid grid-cols-3 gap-2">
          {["Flower", "Edible", "Vape"].map((item) => (
            <div key={item} className="rounded-lg border border-white/10 bg-white/5 p-2 text-center text-[11px] text-slate-300">
              {item}
            </div>
          ))}
        </div>
      ...
      <HeroPanel className="bottom-8 right-2 hidden w-60 md:block" delay={0.56}>
        <MiniStatus title="Intent Signal" value="+34%" icon={BarChart3} />
        <div className="mt-4 flex h-20 items-end gap-2">
          {[48, 72, 40, 90, 62, 82].map((height, index) => (
            <span
              key={index}
              className="flex-1 rounded-t-md bg-gradient-to-t from-violet-tlx to-cyan-tlx opacity-80"
    ...
    <section className="relative overflow-hidden py-10 sm:py-14">
      <div className="section-shell">
        <motion.div
          ...
          className="relative overflow-hidden rounded-[2rem] border border-cyan-tlx/20 bg-[radial-gradient(circle_at_18%_0%,rgba(0,215,232,0.14),transparent_34%),linear-gradient(145deg,rgba(255,255,255,0.07),rgba(255,255,255,0.018))] p-5 shadow-[0_34px_120px_rgba(0,0,0,0.32)] sm:p-8"
        >
          <div className="absolute inset-0 soft-grid opacity-20" />
          <div className="relative grid items-center gap-8 lg:grid-cols-[0.55fr_0.45fr]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-tlx">Product proof earlier</p>
              <h2 className="mt-3 max-w-4xl text-4xl font-semibold leading-tight text-white sm:text-5xl lg:text-6xl">
                The app is the proof: one product record moving through the store.
              </h2>
              <p className="mt-5 max-w-3xl text-lg leading-8 text-muted">
                TLX connects chemistry intake, guided matching, serve-queue context, and retail output before the shopper ever reaches checkout.
              </p>
              <div className="mt-7 grid gap-3 sm:grid-cols-2">
                {["Chemistry reviewed", "Fit explained", "Handoff ready", "Retail surfaces synced"].map((item, index) => (
                  <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.045] p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-muted">Step {index + 1}</p>
                    <p className="mt-2 text-base font-semibold text-white">{item}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="h-[260px] overflow-hidden rounded-[1.5rem] border border-cyan-tlx/20 bg-cyan-tlx/[0.035] sm:h-[360px] lg:h-[430px]">
              <LateStageVisual src="/generated/premium-workflow-console-v2.png" alt="Transparent TLX product proof workflow" />
            </div>
          </div>
        </motion.div>
      </div>
  ...
  const cards = [
    {
      visual: "product",
      label: "Product Media",
      title: "Product imagery becomes part of the intelligence record.",
      text: "COA data, format, effect language, product media, and retail context sit together instead of living in separate tools."
    },
    {
      visual: "kiosk",
      label: "Customer Surface",
      title: "Guided discovery can feel branded and premium.",
      text: "Customers move through goals, refiners, details, and shortlists without the experience feeling like a generic menu."
    },
    {
      visual: "display",
      ...
      {cards.map((card) => (
        ...
        >
          <div className="relative h-72 overflow-hidden bg-cyan-tlx/5">
            <PremiumVisual kind={card.visual as "product" | "kiosk" | "display"} />
...
function ProofControlRoom() {
  return (
    <div className="relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.07),rgba(255,255,255,0.025))] p-4 shadow-[0_30px_90px_rgba(0,0,0,0.36)] sm:rounded-[2rem] sm:p-5">
      <div className="absolute inset-0 soft-grid opacity-25" />
      <div className="relative mb-4 h-[210px] overflow-hidden rounded-[1.35rem] border border-cyan-tlx/20 bg-cyan-tlx/[0.035] sm:h-[310px] sm:rounded-3xl">
        <LateStageVisual src="/generated/premium-workflow-console-v2.png" alt="Transparent TLX operating layer workflow proof" />
      </div>
    ...
    <ProductMockupPanel title="Explore Guided Match" badge="Live Inventory">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
          {["Sleep", "Relaxation", "Focus", "Mood Lift", "Comfort"].map((goal, index) => (
            <span key={goal} className={`rounded-xl border px-3 py-2 text-center text-xs ${index === 1 ? "border-cyan-tlx/35 bg-cyan-tlx/12 text-cyan-100" : "border-white/10 bg-white/5 text-muted"}`}>
              {goal}
            </span>
          ))}
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          {["Flower profile", "Dose-aware edible", "Vape profile"].map((name, index) => (
            <div key={name} className={`relative overflow-hidden rounded-2xl border p-4 ${index === 0 ? "border-cyan-tlx/30 bg-cyan-tlx/[0.08]" : "border-white/10 bg-white/[0.045]"}`}>
              <span className="absolute right-3 top-3 z-10 rounded-full border border-white/10 bg-obsidian/70 px-2.5 py-1 text-[10px] font-semibold text-cyan-100">{index === 0 ? "82" : "Alt"}</span>
              <div className="relative mb-3 h-20 overflow-hidden rounded-xl border border-white/10 bg-cyan-tlx/[0.05] sm:h-28">
                <PremiumVisual kind={index === 1 ? "kiosk" : "product"} compact />
                <div className="absolute inset-0 bg-gradient-to-t from-obsidian/70 to-transparent" />
              </div>
              <p className="text-sm font-semibold text-white">{name}</p>
              <p className="mt-1 text-xs text-muted">{index === 0 ? "Product Match" : "Alternative"}</p>
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-gradient-to-r from-cyan-tlx to-violet-tlx" style={{ width: `${82 - index * 14}%` }} />
              </div>
            </div>
          ))}
        </div>
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
          <p className="rounded-2xl border border-cyan-tlx/20 bg-cyan-tlx/8 p-4 text-sm leading-6 text-cyan-50">
            Lab profile and format suggest a relaxation-oriented option with clear dosage context and in-stock alternatives.
          </p>
          <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-4 text-sm text-slate-200">
            <p className="text-xs uppercase tracking-[0.18em] text-muted">Action</p>
            <p className="mt-2 font-semibold text-white">Compare + handoff</p>
          </div>
        </div>
      </div>
    </ProductMockupPanel>
  ...
  return (
    <ProductMockupPanel title="Retail Presentation Studio" badge="Print Ready">
      <div className="grid gap-4 lg:grid-cols-[0.78fr_1.22fr]">
        <div className="space-y-3">
          {["Background selector", "Card layout", "Operator logo", "Print / export"].map((control, index) => (
          ...
          <div className="rounded-xl border border-cyan-tlx/20 bg-cyan-tlx/[0.07] px-3 py-3 text-sm">
            ...
            <p className="mt-1 text-xs text-muted">Shelf card, QR card, menu tile</p>
          </div>
        </div>
        <div className="scanline overflow-hidden rounded-3xl border border-cyan-tlx/25 bg-gradient-to-br from-cyan-tlx/[0.16] via-blue-tlx/[0.08] to-violet-tlx/[0.16] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
          <div className="relative mb-5 h-28 overflow-hidden rounded-2xl border border-white/10 bg-cyan-tlx/[0.05] sm:h-36">
            <PremiumVisual kind="display" compact />
            <div className="absolute inset-0 bg-gradient-to-t from-obsidian/[0.72] to-transparent" />
          </div>
          <p className="text-xs uppercase text-cyan-100">Premium retail card</p>
          <h4 className="mt-3 text-2xl font-semibold text-white">Flower Profile</h4>
          <p className="mt-2 text-sm text-slate-300">Flower | Available | QR Ready</p>
          <div className="mt-5 grid grid-cols-3 gap-2 text-center text-xs">
            {["22% THC", "Myrcene", "Relax"].map((item, index) => (
              <span key={item} className={`rounded-xl px-2 py-3 text-slate-200 ${index === 2 ? "bg-cyan-tlx/[0.14]" : "bg-black/30"}`}>{item}</span>
            ))}
          </div>
          <div className="mt-4 flex items-center justify-between rounded-2xl border border-white/10 bg-black/25 px-4 py-3">
            <span className="text-xs font-semibold uppercase tracking-[0.18em] text-muted">QR continuation</span>
  ...
  return (
    <Section id="operators" eyebrow="Operator value" title="Built for operators, not just interfaces." intro="TerpLogix helps dispensaries reduce product-data chaos, speed up sales-floor workflows, improve staff confidence, generate better signage and menus, synchronize product surfaces, and understand customer intent before the sale.">
      <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {operatorValues.map(([title, text, Icon]) => (
          <FeatureCard key={title} title={title} text={text} icon={Icon} />
        ))}
      </motion.div>
      <div className="mt-8 rounded-3xl border border-cyan-tlx/20 bg-cyan-tlx/8 p-7 text-2xl font-semibold text-white">
        TLX helps operators turn product truth into sales-floor performance.
      </div>
    </Section>
  );
}
...
function ExperienceBridge() {
  return (
    <Section id="experience" eyebrow="Budtender + customer experience" title="Better discovery. Cleaner handoffs. More confident service." intro="The Serve Queue connects customer exploration to real floor service so staff receive selected products, goals, context, and shortlist before the conversation starts.">
      <div className="grid gap-6 lg:grid-cols-[1fr_0.6fr_1fr]">
        <ExperienceList title="Customer side" items={["Browse by goals and preferences", "Compare products without menu overwhelm", "Save favorites", "Build a shortlist", "Continue on mobile through Discover", "Send picks to a budtender"]} />
        <div className="glass flex flex-col items-center justify-center rounded-3xl p-6 text-center">
          <ClipboardCheck className="h-12 w-12 text-cyan-tlx" />
          <h3 className="mt-4 text-3xl font-semibold text-white">Serve Queue</h3>
          <p className="mt-3 text-base leading-7 text-muted">Customer intent becomes staff context.</p>
        </div>
        <ExperienceList title="Budtender side" items={["See what the customer explored", "Receive context before the conversation", "Compare products quickly", "Explain product differences", "Recommend alternatives", "Serve with more confidence"]} />
      </div>
    </Section>
  );
}
...
function ExperienceList({ title, items }: { title: string; items: string[] }) {
  return (
    <div className="glass rounded-3xl p-6">
      ...
      <div className="mt-5 space-y-3">
        {items.map((item) => (
          <p key={item} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3 text-base text-slate-300">
    ...
    <Section id="customization" eyebrow="White-label retail experience" title="Your retail experience, powered by TLX." intro="Stores can customize logos, colors, customer-facing views, kiosk surfaces, menus, signage, and print materials so the experience feels native to their retail identity while TLX powers the intelligence underneath.">
      <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        <div>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2">
            {["Custom store branding", "Operator logo", "Brand colors", "Location settings", "Customer surfaces", "Kiosk themes", "Store menus", "Signage styling", "Multi-store control"].map((item) => (
...
function CustomizationPanelMockup() {
  return (
    <ProductMockupPanel title="Operator Brand Studio" badge="White-label Ready">
      <div className="grid gap-4 md:grid-cols-[0.72fr_1fr]">
        <div className="space-y-3">
          {[
            ["Store name", "Your Retail Brand"],
            ["Primary", "cyan"],
            ["Accent", "violet"],
            ["Kiosk theme", "Premium"],
            ["Menu style", "Dense"],
            ["Discover app", "Branded"]
          ].map(([label, value]) => (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {["Clinical", "Boutique", "High Energy", "Minimal"].map((style, index) => (
            <div key={style} className="rounded-2xl border border-white/10 bg-white/[0.045] p-4">
              <div className={`mb-4 h-20 rounded-xl ${index === 0 ? "bg-gradient-to-br from-white/16 to-cyan-tlx/16" : index === 1 ? "bg-gradient-to-br from-signal/20 to-violet-tlx/16" : index === 2 ? "bg-gradient-to-br from-cyan-tlx/24 to-violet-tlx/24" : "bg-gradient-to-br from-white/10 to-white/[0.02]"}`} />
              <p className="text-sm font-semibold text-white">{style}</p>
              <p className="text-xs text-muted">Same TLX data</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <Section id="demo" eyebrow="Request demo" title="Ready to see TLX in action?" intro="See how TerpLogix helps turn product truth, customer intent, and retail context into better guidance, smoother handoffs, smarter merchandising, print-ready signage, synchronized menus, live display surfaces, and actionable analytics.">
      <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="glass relative overflow-hidden rounded-3xl p-8">
          <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-cyan-tlx/10 blur-3xl" />
          <div className="relative">
          <h3 className="text-3xl font-semibold text-white">Built for cannabis retail workflows.</h3>
          ...
          <div className="mt-6 space-y-3">
            {[
              ...
              "Synchronized with retail availability",
              "Product intelligence, signage, menus, handoff, and analytics in one ecosystem"
            ].map((item) => (
          ...
          <div className="mt-8 rounded-3xl border border-cyan-tlx/20 bg-cyan-tlx/8 p-5">
            <p className="text-sm font-semibold uppercase text-cyan-tlx">Demo focus</p>
